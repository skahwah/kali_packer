...
echo "[+] (1/5) Downloading Kali Hashes"
wget http://cdimage.kali.org/kali-images/kali-weekly/SHA256SUMS

# set up some variables
HASH=$(grep "kali-linux-2018-.*.-amd64.iso" SHA256SUMS | awk -F" " {'print $1'})
CURRENT=$(grep "kali-linux-2018-.*.-amd64.iso" SHA256SUMS | awk -F" " {'print $2'} | cut -d "." -f 1)

# download current weeks full ISO image
echo "[+] (2/5) Downloading $CURRENT.iso"
mkdir iso
