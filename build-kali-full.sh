...
# Twitter: @hackerjiv
# 9/21/17
# This downloads the current weekly release of Kali and creates a VM using packer
...
HASH=$(grep "kali-linux-2017-.*.-amd64.iso" SHA256SUMS | awk -F" " {'print $1'})
CURRENT=$(grep "kali-linux-2017-.*.-amd64.iso" SHA256SUMS | awk -F" " {'print $2'} | cut -d "." -f 1)
...
# update kali-2017-2-x64.json with current weeks image name and hash
sed -i .bak "s/kali-linux-2017-.*.-amd64/$CURRENT/g" kali-2017-2-x64.json
sed -i .bak "s/\"iso_checksum\": \".*.\"/\"iso_checksum\": \"$HASH\"/g" kali-2017-2-x64.json

# create a copy of the packer configuration file and remove previous hashes
cp kali-2017-2-x64.json kali-2017-2-x64.json.bak
rm SHA256SUMS

# build the VM and execute any post-processors defined
echo "[+] (3/4) Packing Kali"
./packer build kali-2017-2-x64.json
